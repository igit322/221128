...
router.get('/', async (req, res) => {
    let route = req.app.get('views') + '/ejs/admin/certification/certi_writForm.ejs';
    res.render(route);
});
...
router.post('/', upload.array('file'), async (req, res, next) => {
    try {
        ...
        sql = " insert into certification (certiTitle, certiImage) \
        values(?,?); ";
        param = [req.body.certiTitle, paths[0]];
        // console.log(param)
        ...
        connection.query(sql, param, (err, result) => {
            ...
            res.send('<script>alert("공지사항이 등록되었습니다."); location.href="/admin/certiMain?page=1";</script>');
        });
